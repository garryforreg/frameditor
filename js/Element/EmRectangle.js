...
export class EmRectangle{
    ...
    CreateEm(){
        ...
        let LT = node.getAttr("LT");
        let LW = node.getAttr("LW");
        let cssLineStr = "solid";
        if (LT.toString() === "2"){
            cssLineStr = "dashed";
        }else if(LT.toString() === "3"){
            cssLineStr = "dotted";
        }
        ...
        $recDiv.width(w - (parseInt(LW) * 2));
        $recDiv.height(h -  (parseInt(LW) * 2));
    ...
    ExportXml(){
        ...
        let position =  node.getPosition();
        let id = node.getId();
        let xml = XmlParser.parseFromString("<RT/>", "text/xml");
