...
export class EmImage{
    ...
    CreateEm(){
        ...
        $textBack.css("background-image","url('./images/picEmpty.svg')");
    ...
    ExportXml(){
        ...
        let position =  node.getPosition();
        let id = node.getId();
        let xml = XmlParser.parseFromString("<PT/>", "text/xml");
        ...
        AppendDom(dataXml,"ELEMENTTYPE","PT");
    ...
    InitPrototype() {
        ...
        elFrame.contentWindow.AddProperty("数据源", data.a("DATASET"), DsInfo.filter(item => item.value.startsWith("p")), "dropDownList", function (value) {
