...
export class EmLine{
    ...
    CreateEm(){
        ...
        //计算需要绘制的矩形位置
        let rect =   EmLine.CalculateRectangle(drX0, drY0, drX1, drY1,offset);


        //绘制箭头线的矢量图
        let $svg =  self.createSvgLine(index,id,
        ...
        $point0Div.width(offset * 1.5);
        $point0Div.height(offset * 1.5);
        $point0Div.css("z-index", index + 2)
        ...
        $point1Div.width(offset * 1.5);
        $point1Div.height(offset * 1.5);
    
        $point1Div.css("z-index", index + 3)
                        ...
                        menus: [
                            ...
                            },
                            {
                                ...
                                onClick: function (e) {
                                    ...
                                    console.log("删除 图片控件 " + $svg.attr("id"));
                                }
                            }
                        ]
                    }
    ...
    createSvgLine(zIndex, id, x0, y0, x1, y1, width, height, offset, lineWidth, lineColor, lineType) {
        ...
        $svg.attr('id', id);
        $svg.attr('width', width + offset * 1.3); // 考虑偏移量增加SVG大小
        $svg.attr('height', height + offset * 1.3);
    ...
    ExportXml(){
        let node = this.node;
        let id = node.getId();
        let xml = XmlParser.parseFromString("<LN/>", "text/xml");
        let x0 =  parseInt(node.a("X0"));
        let y0 =  parseInt(node.a("Y0"));

        let w = parseInt(node.a("X1")) - x0;
        let h = parseInt(node.a("Y1")) - y0;
        AppendDom(xml, "ID", id);
        AppendDom(xml, "TP", 11);
        AppendDom(xml, "X0", x0);
        AppendDom(xml, "Y0", y0);
